...
function AMConsentList()
{
  require('DBConnect2010.inc');

  $AMquery = "SELECT child_lastname, child_firstname, child_grade, child_classassignment, child_medicalrelease, child_photorelease 
              FROM tst_child_info 
              WHERE ((child_session = 1) AND ((child_medicalrelease<2) OR (child_photorelease<2)))   
...
print $AMquery;
  ...
  $displayresults  = "<B>" . mysqli_num_rows($rs) . " children in the AM session.</B><BR><BR>";
  $displayresults .= "<table border='01' cellpadding='2'>";
  $displayresults .= "<tr>
                          <th>Child Name</th>
                          <th>Class Assignment</th>
                          <th>Medical Release</th>
                          <th>Photo Release</th>
                          </tr>";
...
$ctr = 0;
  ...
  //  $displayresults .= "<td>" . $grades[$resultVars[child_grade]] . "</td>";
    ...
    $displayresults .= "<td>" . $resultVars['child_medicalrelease'] . "</td>";
    $displayresults .= "<td>" . $resultVars['child_photorelease'] . "</td>";
    $displayresults .= "</tr>";
...
function PMConsentList()
{
  require('DBConnect2010.inc');

   $PMquery = "SELECT child_lastname, child_firstname, child_grade, child_classassignment, child_medicalrelease, child_photorelease 
               FROM tst_child_info 
  ...
  $displayresults  = "<B>" . mysqli_num_rows($rs) . " children in the PM session.</B><BR><BR>";
  $displayresults .= "<table border='01' cellpadding='2'>";
  $displayresults .= "<tr>
                          <th>Child Name</th>
                          <th>Class Assignment</th>
